    ...
    public class AppNotificationsManager
    {
        ...
        public async void ShowUserNotification(string type, string conversationId, string username, string message, string id, string ChatId, string avatar, string color, int counterUnreadMessages = 1)
        {
            try
            {
                ...
                var remoteInput = new AndroidX.Core.App.RemoteInput.Builder("key_text_reply").SetLabel(mContext.GetString(Resource.String.Lbl_Reply)).Build();
                ...
                if (ListUtils.MessageUnreadList != null)
                {
                    if (ListUtils.MessageUnreadList.Count > 0)
                    {
                        ...
                        { 
                            NotificationCompat.Action action = new NotificationCompat.Action.Builder(Resource.Drawable.icon_chat_reply, mContext.GetString(Resource.String.Lbl_Reply), resultPendingIntent).AddRemoteInput(remoteInput).Build();
        ...
        public void ShowUpDownNotification(string userName, string messageId, string userId, string chatId)
        {
            try
            {
                ...
                if (Build.VERSION.SdkInt >= BuildVersionCodes.O)
                {
                    ...
                    MNotifyBuilder = new NotificationCompat.Builder(mContext, channelId).SetVisibility(NotificationCompat.VisibilityPublic).SetColor(ContextCompat.GetColor(mContext, Resource.Color.accent)).SetSmallIcon(Resource.Mipmap.icon).SetChannelId(channelId).SetContentIntent(resultPendingIntent).SetCategory(NotificationCompat.CategoryProgress);
                ...
                {
                    channelId = userId; // The id of the channel.
                    MNotifyBuilder = new NotificationCompat.Builder(mContext, channelId).SetVisibility(NotificationCompat.VisibilityPublic).SetColor(ContextCompat.GetColor(mContext, Resource.Color.accent)).SetSmallIcon(Resource.Mipmap.icon).SetPriority((int)NotificationPriority.Low).SetContentIntent(resultPendingIntent).SetCategory(NotificationCompat.CategoryProgress);
                }
