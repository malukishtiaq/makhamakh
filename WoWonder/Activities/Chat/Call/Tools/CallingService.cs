    ...
    public class CallingService : Service, ProximitySensor.IDelegate, AudioManager.IOnAudioFocusChangeListener
    {
        ...
        private void RejectCall(Intent intent)
        {
            try
            {
                ...
                {
                    case "Twilio_video_call":
                        PollyController.RunRetryPolicyFunction(new List<Func<Task>> { async () => await RequestsAsync.Call.DeclineCallTwilioAsync(CallConstant.CallUserObject.Data.Id, TypeCall.Video) });
                        ...
                        break;
                    case "Twilio_audio_call":
                        PollyController.RunRetryPolicyFunction(new List<Func<Task>> { async () => await RequestsAsync.Call.DeclineCallTwilioAsync(CallConstant.CallUserObject.Data.Id, TypeCall.Audio) });
                        ...
                        break;
                    case "Agora_video_call_recieve":
                        PollyController.RunRetryPolicyFunction(new List<Func<Task>> { async () => await RequestsAsync.Call.DeclineCallAgoraAsync(CallConstant.CallUserObject.Data.Id) });
                        ChatTabbedMainActivity.AddCallToListAndSend("Cancel", GetText(Resource.String.Lbl_Missing), TypeCall.Video, CallConstant.CallUserObject);
                        break;
                    case "Agora_audio_call_recieve":
                        PollyController.RunRetryPolicyFunction(new List<Func<Task>> { async () => await RequestsAsync.Call.DeclineCallAgoraAsync(CallConstant.CallUserObject.Data.Id) });
