    ...
    internal static class ApiRequest
    {
            ...
            if (Methods.CheckConnectivity())
            {
                ...
                await Task.Factory.StartNew(() =>
                {
                    try
                    {
                        ...
                        switch (result.PageSubCategories?.SubCategoriesList?.Count)
                        {
                            case > 0:
                                {
                                    //Sub Categories Page
                        ...
                        switch (result.GroupSubCategories?.SubCategoriesList?.Count)
                        {
                            case > 0:
                                {
                        ...
                        switch (result.ProductsSubCategories?.SubCategoriesList?.Count)
                        {
                            case > 0:
                                {
        ...
        public static async Task GetArchivedChats()
        {
            if (Methods.CheckConnectivity() && !string.IsNullOrEmpty(UserDetails.AccessToken) && AppSettings.EnableChatArchive)
            {
                ...
                var (apiStatus, respond) = await RequestsAsync.Message.GetArchivedChatsAsync(fetch).ConfigureAwait(false);
